/*
 * @Author: Zkiki
 * @Date: 2023-01-02 15:57:04
 * @LastEditTime: 2023-04-14 08:36:13
 * @LastEditors: Please set LastEditors
 * @FilePath: \leetcodeStudy\src\4.数组\18.[ 1013 ] 将数组分成和相等的三个部分.js
 * @Description: 
 */
/**
 * * 题目名称：将数组分成和相等的三个部分
 * * 题目地址：https://leetcode-cn.com/problems/partition-array-into-three-parts-with-equal-sum
 * 给你一个整数数组 arr，只有可以将其划分为三个和相等的 非空 部分时才返回 true，否则返回 false。

形式上，如果可以找出索引 i + 1 < j 且满足 (arr[0] + arr[1] + ... + arr[i] == arr[i + 1] + arr[i + 2] + ... + arr[j - 1] == arr[j] + arr[j + 1] + ... + arr[arr.length - 1]) 就可以将数组三等分。

示例 1：

输入：arr = [0,2,1,-6,6,-7,9,1,2,0,1]
输出：true
解释：0 + 2 + 1 = -6 + 6 - 7 + 9 + 1 = 2 + 0 + 1
示例 2：

输入：arr = [0,2,1,-6,6,7,9,-1,2,0,1]
输出：false
示例 3：

输入：arr = [3,3,6,5,-2,2,5,1,-9,4]
输出：true
解释：3 + 3 = 6 = 5 - 2 + 2 + 5 + 1 - 9 + 4
 */

// * 思路：重点是连续的，切分成三部分的和，将数组之和求出，除以3份
/**
 * @param {number[]} arr
 * @return {boolean}
 */
var canThreePartsEqualSum = function (arr) {
    let sum = arr.reduce((a, b) => a + b, 0);
    if (sum % 3 !== 0) return false;
    const fraction = sum / 3
    let fractionSum = 0;
    let count = 3
    for (let k in arr) {
        if (fractionSum + arr[k] === fraction) {
            if (count > 0) {
                count--;
                fractionSum = 0;
            } else {
                fractionSum += arr[k]
            }
        } else {
            fractionSum += arr[k]
        }
    }

    if (count === 0 && fractionSum === 0) {
        return true
    } else {
        return false
    };
}



let test3 = [3, 3, 6, 5, -2, 2, 5, 1, -9, 4]
console.time('执行用时');
// console.log(canThreePartsEqualSum(test));
console.log(canThreePartsEqualSum(test2));
// console.log(canThreePartsEqualSum(test3));
console.timeEnd('执行用时');